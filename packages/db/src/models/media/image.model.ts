 ...
 * images. Exclude `data` from list queries
 * (`.select("-data")`) to avoid loading every blob into memory.
 */
...
/** Fields a caller supplies when storing a new image. */
...
export const ImageModel: Model<Image> = (models.Image as Model<Image>) ?? model<Image>("Image", imageSchema)
