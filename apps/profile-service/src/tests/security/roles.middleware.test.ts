...
describe("requirePermission", () => {
  it("allows when user has matching permissions", async () => {
    ...
    app.use((req: Request, _res: Response, next: NextFunction) => {
      req.user = { id: "user-1", role: "user", permissions: ["profile:read"] } as Request["user"]
      next()
    })
    app.get("/test", requirePermission("profile:read"), (_req: Request, res: Response) => {
      res.json({ ok: true })
    })
  ...
  it("denies when user lacks permissions", async () => {
    ...
    app.get("/test", requirePermission("profile:admin"), (_req: Request, res: Response) => {
      res.json({ ok: true })
    })
    const res = await request(app).get("/test")
    expect(res.status).toBe(403)
    expect(res.body).toEqual({ success: false, error: "Forbidden", required: "profile:admin" })
  })
...
describe("requireSelfOrPermission", () => {
  it("allows self", async () => {
    const app = express()
    app.use((req: Request, _res: Response, next: NextFunction) => {
      req.user = { id: "user-1", role: "user", permissions: ["profile:admin"] } as Request["user"]
      next()
    })
  ...
  it("allows user with elevated permission for non-self", async () => {
    ...
    app.get(
      "/test/:id",
      requireSelfOrPermission("id", "profile:admin"),
...
describe("requireOwnership", () => {
  it("allows owner to access resource", async () => {
    ...
    app.get(
      "/test/:id",
      requireOwnership(async (req) => ({ authorId: req.params.id }) as never, "profile:admin"),
      (_req: Request, res: Response) => {
        res.json({ ok: true })
      }
    )
  ...
  it("allows user with elevated permission for non-owned resource", async () => {
    ...
    app.get(
      "/test/:id",
      requireOwnership(async () => ({ authorId: "other-user" }) as never, "profile:admin"),
      (_req: Request, res: Response) => {
        res.json({ ok: true })
      }
    )
  ...
  it("returns 404 when resource not found", async () => {
    ...
    app.get(
      "/test/:id",
      requireOwnership(async () => null as never, "profile:admin"),
      (_req: Request, res: Response) => {
        res.json({ ok: true })
      }
    )
